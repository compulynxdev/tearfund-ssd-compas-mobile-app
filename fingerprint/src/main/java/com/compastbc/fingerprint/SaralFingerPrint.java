 ...
 */
public final class SaralFingerPrint implements FingerprintCallback {
    private static final String TAG = "SaralFingerPrint";

    private static SaralFingerPrint instance;
    ...
    private SaralFingerPrint(Context context) {
        this.context = context;
        initSaralFp(false);
    }

    public static SaralFingerPrint getInstance(Context context) {
        if (instance == null)
            instance = new SaralFingerPrint(context);
    ...
    private void initSaralFp(boolean isReinitialise) {
        close();

        pDialog = null;
        pDialog = new MaterialDialog.Builder(context)
                .title(isReinitialise ? context.getString(R.string.error_fp_re_scanner) : context.getString(R.string.error_fp_scanner))
        ...
        try {
            new Thread(() -> {
                scanner = FingerprintScanner.getInstance();
                scanner.powerOn();
                initSecugen(() -> handler.post(() -> pDialog.dismiss()));
            }).start();
        } catch (Exception e) {
            e.printStackTrace();
            handler.post(() -> pDialog.dismiss());
        }
    }

    private void initSecugen(SecugenInitSuccess secugenInitSuccess) {
        /*Init secugen fp*/
        ...
        if (secugenInitSuccess != null) secugenInitSuccess.onSuccess();
    }
    ...
    public void captureFingerPrintWithEncodeData(FingerprintReaderInit.FingerPrintDataCallback callback) {
        ...
        } else callback.onFingerPrintQualityError();
    }
    ...
    public void captureFingerPrintWithEncodeData(String TAG, FingerprintReaderInit.FingerPrintDataTagCallback callback) {
        ...
        } else callback.onFingerPrintQualityError(TAG);
    }
    ...
    public void close() {
        ...
        /*other fp device variable destroy*/
        if (scanner != null) {
            scanner.close();
            //scanner.powerOff();
        }
    ...
    public interface SecugenInitSuccess {
        void onSuccess();
    }
}
